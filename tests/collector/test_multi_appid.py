...
class TestMultiAppidScenarios:
    ...
    def test_get_games_data_mixed_success_failure_with_raise_on_error(
        ...
        with (
            patch.object(SteamStore, "fetch", side_effect=mock_get),
            patch.object(HowLongToBeat, "_get_search_token", return_value="mock_token"),
        ):
            collector = Collector()

            # Should raise on the first failure
            with pytest.raises(GameNotFoundError):
                collector.get_games_data(["12345", "99999"], raise_on_error=True)
